...
INTERVAL = 1.05
DAILY_LIMIT = 8_000
# Cool-down between retries when the WAF answers empty HTTP 400 (blocking
# window).  Chosen so 5 consecutive blocked targets stop the run in ~20min.
BLOCK_COOLDOWN_SECONDS = 120.0
PLACE_CODES = {
    ...
    def fetch(self, target: Target) -> CachedPage:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = exc
                if status == 400:
                    # netkeiba's WAF serves empty HTTP 400 in rolling blocking
                    # windows once cumulative volume is noticed (2026-07-27:
                    # Google-indexed, certainly-existing pages 400ed for every
                    # client variant from this host, then recovered).  These
                    # are NOT page-level truths: never mark the page as a
                    # permanent gap.  Cool down much longer than the normal
                    # backoff, and if the window persists let the consecutive
                    # -failure stop end the run cleanly (resumable).
                    saw_http_400 = True
        ...
        if body is None:
            code = (
                ...
                if isinstance(last_error, MemberContentError)
                    ...
                    "http_400_blocked_window" if saw_http_400
