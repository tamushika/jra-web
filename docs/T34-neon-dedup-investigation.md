...
## 2026-07-16 追記: v2.2再dry-run結果と2025年全件監査 (Fable 5 上位モデル)

### 再dry-run (READ-ONLY・無変更)

bundle: `outputs/t34b/20260716T101022.996693Z_dry_run` / manifest_sha256
`7b7abe8fea3efd48be13dbca7474ccb1176d9963b70db74da00eebb44737bf4c` / plan_sha256
`4a5d9cdd47b1f248f5cdc18c1e57454fddf2f88754128c450e481242962e6d41`。

- v2.2のtime修正は成功: **`time` 709/709一致** (7/14は709全滅)。全15比較列が一致。
- 分類: **KEEP 295 / MOVE 414 / DELETE 0 / UNRESOLVED 7**。plan `NOT_APPLICABLE`
  (理由: UNRESOLVED、chain/cycle未対応、最終キー衝突4件)。

### UNRESOLVED 7行の正体 = 競走中止馬 (netkeiba照合で5行のrace_num誤りを確定)

7行すべて rank NULL・time `"----"` の**競走中止**馬。ability.dbは中止馬を収録しない
(truth 0件は設計どおり)。netkeiba馬ページ (nk_horse_ids のIDで直接取得) と照合:

| 馬 | Neon | netkeiba実レース | 判定 |
|---|---|---|---|
| ミグラテール | 小倉250208 R4 u3 | 小倉**R5** u3 (障2860・雪) 中止 | race_num誤り → R5 |
| トーアマリシテン | 小倉250208 R4 u5 | 小倉**R5** u5 中止 | race_num誤り → R5 |
| アルカンサス | 小倉250208 R4 u7 | 小倉**R5** u7 中止 | race_num誤り → R5 |
| スターコンパス | 中山250412 R1 u9 | 中山**R3** u9 (ダ1800) 中止 | race_num誤り → R3 |
| チンプンカンプン | 福島251116 R4 u9 | 福島**R5** u9 (障2750) 中止 | race_num誤り → R5 |
| ハーフェズ | 新潟250518 R3 u11 | 新潟R3 u11 中止 | **現状正しい** |
| パルピタシオン | 福島251116 R4 u7 | 福島R4 u7 中止 | **現状正しい** |

裏取り: ability.dbの各レース頭数が「netkeiba実頭数 − 中止頭数」と全件一致
(小倉R5: 9−3=6、福島R5: 11−1=10、中山R3: 13−1=12、新潟R3: 13−1=12)。
移動先キーはability truth・Neonとも空きを確認済み (例外: 中山250412 R3 u9 は
モーニングマジック行が占有 — 下記)。

### 「見えない誤行」の発見と2025年全件監査

中山250412 R3 u9 の占有者モーニングマジックは truth では **R1 u9・3着・116.0秒**
(Neon行のtime `1560`=116.0秒と一致)。つまり**キーは重複していないがrace_numが誤っている行**で、
重複起点の候補抽出には掛からない。これを受け2025年の完全キー行44,634行を
ability truth (47,497行) と全件照合した:

| 区分 | 行数 |
|---|---|
| 馬名一致 | 41,535 |
| **キーは一意だが馬名不一致 (誤行)** | **2,589** (重複676行の誤側を含む) |
| キーがtruthに無く結果あり | 180 |
| キーがtruthに無く非出走マーカー (rank NULL・time `----`) | 330 (大半は正当な中止・取消行) |

不一致はレース単位のrace_num入れ替え (例: 福島251115 R7、planの連鎖80組と同型) が主で、
特定開催日に集中 (福島250420: 77行、中山250330: 75行 等)。**重複338組は誤登録の
氷山の一角**であり、dry-run planの「chain/cycle」80組もこの入れ替えの断面。

### 2026年は無事 (評価基盤への波及なし)

同じ照合を2026年 (9,384行) に実施: **不一致0**。特に1〜4月はT31でnetkeibaから独立に
再構築したtruthに対し1,003/1,003一致 (5月以降はNeon→ability同期のため循環参照だが、
独立検証できる期間がクリーンなので登録バグの2026年再発は否定的)。
**ability.dbのOOS評価基盤 (2025凍結・2026H1/H2) は汚染されていない。**

### 是正計画 (改定)

1. **手動是正 6行** (上位モデル実施・ユーザー承認+CSVバックアップ必須、7/4是正と同手順):
   上表の5行のrace_num修正 + モーニングマジック R3 u9→R1 u9 (スターコンパスとの入替)。
   ツールのtruth (ability.db) では証明不能なため手動でしか直せない。
2. **SPEC-T34b v2.3** (Codex実装): 候補抽出を「重複キー」から「truth不一致行全件」へ一般化、
   検証済みMOVEのみで構成される連鎖・循環の一括最終状態適用、非mutation分類
   (`NON_RUNNER_KEEP`/`UNVERIFIED_KEEP`) の追加。これで誤行2,589行が同一の安全機構で直る。
3. 以降は従来どおり: 再dry-run → 上位モデルレビュー → 承認apply → 部分UNIQUE index →
   postcheck → 新updater配備。

## 状態
- 根本原因 = 特定済み。7/4の5組は是正済み (Neon)。登録アプリ再発防止コード = コミット済 (7e5ce59)。
- **migration は未適用** (当初設計が実データに不適合と判明 → SPEC-T34b で再設計)。
- v2.2 (Neon圧縮timeパーサ) = レビュー済・コミット済。再dry-runでtime 709/709一致を実証。
- **手動是正6行 = 実施済み** (2026-07-16 ユーザー承認・Fable 5実施。バックアップ
  `backups/t34b_neon_dnf_racenum_fix_20260716.csv`。是正後dry-run: UNRESOLVED 7→2、
  最終キー衝突0、重複キー31→27で想定どおり)。
- **未実施**: ①SPEC-T34b v2.3.1 のCodex実装 (誤行2,589行への一般化。Codex仕様レビュー
  反映済み・本文統合済み)、②実装レビュー→本番dry-run→承認apply、
  ③部分ユニークインデックスの本番適用→新updater配備。
