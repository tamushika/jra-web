...
1. **分割**: 学習2021-23 / 選抜2024 (L2とグループ選択のみ、事前固定の小グリッド) /
   ...
   **グリッドは台帳登録済み (experiment_id `T41-lowcost-feature-pack-v1`)**:
   L2 ∈ {0.3, 1.0, 3.0} × 構成6種 = 18候補。これ以外の候補を評価しない。
   **historical benchmarkのモデルは、2024で選抜した構成・L2のまま2021-24で
   final refit したもの** (T10/T33と同じ慣行)。参考値として21-23学習版も併記
