...
## ✅ バックフィル完走・検証完了 (2026-07-11) — sire_pts 採用済み

**バックフィルは32,497頭で完走 (検証期間カバレッジ100%)。日課は終了**
(今後の新馬は出馬表スクレイプ時の受動的蓄積が自動で拾う)。
100%カバレッジでの最終OOS: k=1 32.1 / k=2 51.6 / k=3 64.4 (血統導入前比 k=1 +1.0pt)。

検証の経緯 (81.2%時点):
- sire_pts 係数 +0.21 (オッズに次ぐ第2位の特徴量)
- 2025年OOS: k=1 31.3→31.8 / k=3 63.1→64.6 (+1.8pt) / 平均54.3→55.1
- 副産物: load_runs の列修正で age (−0.09)・weight 等の休眠特徴量も復活
- 残タスク: バックフィル完走 (残り約5,800頭、日課継続でカバレッジ100%へ) と
  下記「併せてやると良いこと」(血統あり再マイニング・書籍血統ルール解禁)
...
- カバレッジの注意: `--status` の全期間カバレッジは2026年含み。**検証が使うのは
  21-25年出走馬のカバレッジ** (backtest_ml 起動ログに表示) で、判定はそちらで行う。
  検証前に `pedigree_store.load_all(refresh=True)` でキャッシュ更新を忘れないこと
