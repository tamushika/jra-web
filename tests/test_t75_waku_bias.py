...
# ─── テストデータビルダ ──────────────────────────────────────────────────────
# 16頭立てレースでの calculate_waku(u, 16) の実際の対応 (SPEC-T80: JRA規則どおり。
# h=16 は q,r=divmod(16,8)=(2,0) なので各枠2頭ずつ):
#   1〜2→1枠, 3〜4→2枠, 5〜6→3枠, 7〜8→4枠, 9〜10→5枠, 11〜12→6枠, 13〜14→7枠, 15〜16→8枠
# → 内(1〜3枠) = 馬番{1..6} / 中(4〜5枠) = 馬番{7..10} / 外(6〜8枠) = 馬番{11..16}
...
# 6レース分の3着以内馬番 (フラットに近いシナリオ (b): in=6, mid=6, out=6)
_PODIUMS_FLAT = [
    [8, 9, 10],
    [11, 12, 13],
    [1, 4, 14],
...
# 外枠集中の6レース分 (過去日用): 16頭立てで馬番14,15,16 (waku=7,8,8=いずれも外) に集中
